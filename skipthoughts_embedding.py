...
class SkipThoughts():
	...
	def encode_train_images(self):
		...
		imgs_train = []
		dataType = "mscoco"
		for image_id in image_list_train:
	...
	def encode_test_images(self):
		...
		imgs_test = []
		dataType = "mscoco"
		for image_id in image_list_test:
			# Resize the images as VGG inputs
	...
	def top_images(self):
		...
		cs_dic = {qID: fc7_dic[qID][0].dot(fc7_dic[qID][1].T)/np.linalg.norm(fc7_dic[qID][0])*np.linalg.norm(fc7_dic[qID][1], axis=1)
		for qID in fc7_dic.keys()}

		# Dictionary mapping question_id from test to top_image
		qID_to_img_nn = {qID: dic_imgID_nn[(qID,test_qID_to_imgID[qID])][np.argmax(cs_dic[qID])]
		for qID in cs_dic.keys()}

		return qID_to_img_nn
